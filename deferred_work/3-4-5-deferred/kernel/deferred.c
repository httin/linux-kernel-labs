...
static long deferred_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case MY_IOCTL_TIMER_CANCEL:
			/* TODO 1: cancel timer */
			if(del_timer(&my_data->timer))
				pr_info("[del_timer] Canceled a activated timer\n");
			else
				pr_info("[del_timer] Canceled a unactivated timer\n");
			break;
...
static int deferred_init(void)
{
	...
	pr_info("[deferred_init] Init module, private_data[@dev] at %p\n", &dev);
	...
	/* @TIMER_DEFERRABLE 0x00 A deferrable timer will work normally when the
	 * system is busy */
