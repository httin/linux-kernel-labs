...
#include <linux/fs.h>			/* file_operations, inode, ... everything */
#include <linux/cdev.h>			/* register_chrdev_region(), cdev_init(), cdev_add(),... */
#include <asm/io.h>				/* inb() */
#include <linux/uaccess.h>		/* request_region(), put_user() */
#include <linux/ioport.h>
#include <linux/interrupt.h>	/* request_irq() */
#include <linux/spinlock.h>		/* spinlock_t, spin_lock_init() */
...
DECLARE_TASKLET(kb_tasklet_struct, keyboard_tasklet_bh, 0);
...
void keyboard_tasklet_bh(unsigned long hits) 
{
	...
	pr_info("IRQ %d: scancode=0x%x (%u) ch=%c\n",
			I8042_KBD_IRQ, scancode, scancode, ch);
}
...
static int get_ascii(unsigned int scancode)
{
	...
	if (scancode >= 0x10 && scancode <= 0x2b)
		return *(row2 + scancode - 0x10);
	if (scancode >= 0x1e && scancode <= 0x2)
...
irqreturn_t kbd_interrupt_handle(int irq_no, void *dev_id)
{
	...
	if (is_key_press(scancode)) {
		...
		kb_tasklet_struct.data = scancode;
		/* schedule the tasklet */
  	tasklet_schedule(&kb_tasklet_struct);
	}
			 ...
			 size_t size, loff_t *offset)
{
  ...
  char ch;
	...
	/* disable interrupts for the local processor before obtaining the spinlock */
	spin_lock_irqsave(&data->lock, flags);
	reset_buffer(data);
  *offset = 0;
	/* release spinlock before enabling interrupts for the local processor */
	spin_unlock_irqrestore(&data->lock, flags);

  /* ----- added by Huynh Trung Tin ----- */
  size = (*offset + size > BUFFER_SIZE) ? (BUFFER_SIZE - *offset) : size;
  if (copy_from_user(data->buf + *offset, user_buffer, size) != 0)
    return -EFAULT;
  *offset += size;
  data->count = *offset;
  data->put_idx = *offset;
...
static int kbd_init(void)
{
	...
	cdev_init(&devs[0].cdev, &kbd_fops);	/* initialize character device  */
	cdev_add(&devs[0].cdev, MKDEV(KBD_MAJOR, KBD_MINOR), 1); /* notice to kernel */
