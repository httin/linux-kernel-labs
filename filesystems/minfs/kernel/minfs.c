...
/*
 * additional information (found in struct minfs_super_block (on disk)
 * but not in struct super_block (VFS)) 
 */
...
static struct inode *minfs_iget(struct super_block *s, unsigned long ino)
{
	...
	if (inode == NULL) {
		printk(LOG_LEVEL "%s: error aquiring inode\n", __FUNCTION__);
	 ...
	 * The information about the root inode is found in the 
	 * second block on the disk (the inode with index 1)
	 */
...
static int minfs_fill_super(struct super_block *s, void *data, int silent)
{
	...
	printk(LOG_LEVEL "%s: bh->b_data = %s\n", __FUNCTION__, bh->b_data);
...
out_iput:
	iput(root_inode);
out_bad_inode:
	printk(LOG_LEVEL "%s: bad inode\n", __FUNCTION__);
out_bad_magic:
	printk(LOG_LEVEL "%s: bad magic number %#lx\n", __FUNCTION__, ms->magic);
	brelse(bh);
out_bad_sb:
	printk(LOG_LEVEL "%s: error reading buffer_head\n", __FUNCTION__);
out_bad_blocksize:
	printk(LOG_LEVEL "%s: bad block size\n", __FUNCTION__);
	s->s_fs_info = NULL;
	kfree(sbi);
	return ret;
}
		...
		int flags, const char *dev_name, void *data)
{
	...
	/* mounts a filesystem stored on a block device */
...
static struct file_system_type minfs_fs_type = {
	...
	.mount 		= minfs_mount,
	/* destroying a superblock filesystem with disk support */
	.kill_sb	= kill_block_super,
	/* filesystem needs a disk */
...
static int __init minfs_init(void)
{
	...
	if (err) {
		...
		printk(LOG_LEVEL "%s: register_filesystem succeeded\n", __FUNCTION__);
...
static void __exit minfs_exit(void)
{
	...
	printk(LOG_LEVEL "minfs: unregister filesystem");
}
